<script lang="ts">
	...
	$: filteredData = allVendorPaidData.filter(p => {
		// Search filter
		if (searchQuery.trim()) {
			const query = searchQuery.toLowerCase();
			const matchesSearch = 
				(p.vendor_name && p.vendor_name.toLowerCase().includes(query)) ||
				(p.vendor_id && String(p.vendor_id).toLowerCase().includes(query));
	...
	// Load all vendor paid data with pagination
	async function loadAllVendorPaid() {
		isLoading = true;
		loadingProgress = 0;
		allVendorPaidData = [];
		currentPage = 0;
		...
		try {
			...
			while (hasMoreData) {
				...
				const { data, error } = await supabase
					.from('vendor_payment_schedule')
					.select('id, bill_number, vendor_name, vendor_id, final_bill_amount, paid_date, branch_id, payment_method, payment_reference')
					.eq('is_paid', true)
					.order('paid_date', { ascending: false })
					.range(from, to);

				if (error) {
					console.error('Error loading vendor paid:', error);
					break;
				}

				if (data && data.length > 0) {
					allVendorPaidData = [...allVendorPaidData, ...data];
					currentPage++;
					loadingProgress = Math.min(95, Math.round((allVendorPaidData.length / totalCount) * 100));
			...
			loadingProgress = 100;
			console.log('✅ Loaded all vendor paid:', allVendorPaidData.length, 'records');
		} catch (error) {
			console.error('Error loading vendor paid:', error);
		} finally {
			isLoading = false;
	...
	function exportToExcel() {
		try {
			const excelData = filteredData.map(payment => ({
				'Bill #': payment.bill_number || 'N/A',
				'Vendor Name': payment.vendor_name || 'N/A',
				'Vendor ID': payment.vendor_id || 'N/A',
				'Amount': payment.final_bill_amount || 0,
				'Paid Date': formatDate(payment.paid_date),
			...
			XLSX.utils.book_append_sheet(workbook, worksheet, 'Vendor Paid');
			...
			const filename = `Vendor_Paid_${dateStr}.xlsx`;
	...
	onMount(async () => {
		await loadBranches();
		await loadAllVendorPaid();
	});
</script>

<div class="all-vendor-paid-container">
	{#if isLoading}
		<div class="loading-section">
			<div class="loading-spinner"></div>
			<div class="loading-text">Loading all vendor paid records...</div>
			<div class="progress-bar">
				<div class="progress-fill" style="width: {loadingProgress}%"></div>
			</div>
			<div class="progress-text">{allVendorPaidData.length} of {totalCount} records ({loadingProgress}%)</div>
		</div>
	{:else}
		<div class="header-section">
			<h2>📦 All Vendor Paid Transactions</h2>
			<div class="stats">
				<span class="stat-badge">{filteredData.length} / {allVendorPaidData.length} Records</span>
				<button class="export-btn" on:click={exportToExcel}>📄 Export</button>
				<button class="refresh-btn" on:click={loadAllVendorPaid}>🔄 Refresh</button>
			</div>
		</div>
