...
function createTaskStore() {
  ...
  return {
    ...
    async createQuickTaskAssignments(
      assignments: Array<{
      ...
      try {
        const { data, error } =
          await db.quickTaskAssignments.createMultiple(assignments);
        if (error) {
          const errorMessage =
            error.message || "Failed to create quick task assignments";
        ...
        console.log(
          "✅ [Store] Quick task assignments created successfully:",
          data,
        );
        return { success: true, data };
      } catch (error) {
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Unexpected error creating quick task assignments";
        console.error(
          "❌ [Store] createQuickTaskAssignments exception:",
          error,
        );
        update((state) => ({ ...state, error: errorMessage }));
        return { success: false, error: errorMessage };
    ...
    async submitQuickTaskCompletion(
      ...
      try {
        ...
        if (error) {
          const errorMessage =
            error.message || "Failed to submit quick task completion";
        ...
        console.log(
          "✅ [Store] Quick task completion submitted successfully:",
          data,
        );
        return { success: true, data };
      } catch (error) {
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Unexpected error submitting quick task completion";
    ...
    async getQuickTaskAssignments(quickTaskId: string) {
      try {
        const { data, error } =
          await db.quickTaskAssignments.getByTaskId(quickTaskId);
        if (error) {
          const errorMessage =
            error.message || "Failed to load quick task assignments";
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Unexpected error loading quick task assignments";
...
export const createQuickTaskAssignments =
  taskStore.createQuickTaskAssignments.bind(taskStore);
export const submitQuickTaskCompletion =
  taskStore.submitQuickTaskCompletion.bind(taskStore);
export const getQuickTaskAssignments =
  taskStore.getQuickTaskAssignments.bind(taskStore);
