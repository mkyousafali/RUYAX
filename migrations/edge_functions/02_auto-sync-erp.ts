...
//
// Usage:
//   POST with {"branch_id": 5}  ΓåÆ sync specific branch
//   POST with {}                ΓåÆ sync the NEXT branch due for sync (round-robin)
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ΓöÇΓöÇ Get the target branch connection ΓöÇΓöÇ
    let conn: any = null;
    ...
    if (params.branch_id) {
      ...
      // No branch specified ΓåÆ pick the one with the oldest synced_at
      ...
      // Build a map of branch_id ΓåÆ last sync time
