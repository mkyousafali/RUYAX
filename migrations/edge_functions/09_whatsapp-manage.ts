...
serve(async (req: Request) => {
  ...
  try {
    ...
    switch (action) {
      // ΓöÇΓöÇΓöÇ Send Message ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "send_message":
        result = await sendMessage(accessToken, phoneNumberId, params);
        break;

      // ΓöÇΓöÇΓöÇ Send Template Message ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "send_template":
        result = await sendTemplate(accessToken, phoneNumberId, params);
        break;

      // ΓöÇΓöÇΓöÇ Mark Message as Read ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "mark_read":
        result = await markAsRead(accessToken, phoneNumberId, params.message_id);
        break;

      // ΓöÇΓöÇΓöÇ Get Business Profile ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "get_business_profile":
        result = await getBusinessProfile(accessToken, phoneNumberId);
        break;

      // ΓöÇΓöÇΓöÇ Update Business Profile ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "update_business_profile":
        result = await updateBusinessProfile(accessToken, phoneNumberId, params.profile);
        break;

      // ΓöÇΓöÇΓöÇ Create Template ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "create_template":
        result = await createTemplate(accessToken, wabaId, params.template, phoneNumberId);
        break;

      // ΓöÇΓöÇΓöÇ Get Templates ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "get_templates":
        result = await getTemplates(accessToken, wabaId, params.limit, params.after);
        break;

      // ΓöÇΓöÇΓöÇ Delete Template ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "delete_template":
        result = await deleteTemplate(accessToken, wabaId, params.template_name);
        break;

      // ΓöÇΓöÇΓöÇ Get Media URL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "get_media":
        result = await getMediaUrl(accessToken, params.media_id);
        break;

      // ΓöÇΓöÇΓöÇ Upload Media ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "upload_media":
        result = await uploadMedia(accessToken, phoneNumberId, params.file_url, params.mime_type);
        break;

      // ΓöÇΓöÇΓöÇ Get Phone Numbers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "get_phone_numbers":
        result = await getPhoneNumbers(accessToken, wabaId);
        break;

      // ΓöÇΓöÇΓöÇ Test Connection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      case "test_connection":
        result = await testConnection(accessToken, phoneNumberId);
        break;

      // ΓöÇΓöÇΓöÇ Send Broadcast (fire-and-forget with auto-continue) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      // IMPORTANT: Auto-continue happens ONLY here ΓÇö NOT inside sendBroadcast() ΓÇö
      ...
      case "send_broadcast": {
        // Start broadcast in background ΓÇö do NOT await
        sendBroadcast(supabase, accessToken, phoneNumberId, params).then(async (result) => {
          ...
          if (result?.timedOut) {
            console.log(`[Broadcast] Γå¬ Auto-continue: ${result.sent} sent this round, invoking next batch...`);
            ...
            for (let attempt = 1; attempt <= 7; attempt++) {
              try {
                console.log(`[Broadcast] Γå¬ Auto-continue attempt ${attempt}/7...`);
                ...
                if (resp.ok) {
                  console.log(`[Broadcast] Γå¬ Auto-continue invoked successfully (attempt ${attempt})`);
                  break; // success
                } else {
                  const errText = await resp.text().catch(() => "");
                  console.error(`[Broadcast] Γ¥î Auto-continue HTTP ${resp.status} (attempt ${attempt}): ${errText.substring(0, 200)}`);
                  // Retry on non-2xx
                  if (attempt < 7) {
                ...
                console.error(`[Broadcast] Γ¥î Auto-continue fetch error (attempt ${attempt}): ${fetchErr?.message}`);
                // Aggressive backoff on error
                if (attempt < 7) {
                  const delayMs = attempt <= 3 ? 500 * attempt : Math.min(15000, 1000 * Math.pow(2, attempt));
                  console.log(`[Broadcast] Γå¬ Retrying in ${delayMs / 1000}s...`);
                  await new Promise(r => setTimeout(r, delayMs));
                }
              }
            }
      ...
      // ΓöÇΓöÇΓöÇ Slow Ecosystem Retry (3 msg/s, only ecosystem-failed recipients) ΓöÇΓöÇ
      ...
      case "send_eco_retry": {
        sendEcoRetry(supabase, accessToken, phoneNumberId, params).then(async (result) => {
          if (result?.timedOut) {
            console.log(`[EcoRetry] Γå¬ Auto-continue: ${result.sent} sent this round, continuing...`);
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
              try {
                ...
                if (resp.ok) {
                  console.log(`[EcoRetry] Γå¬ Auto-continue invoked (attempt ${attempt})`);
                  break;
                } else {
                  console.error(`[EcoRetry] Γ¥î Auto-continue HTTP ${resp.status} (attempt ${attempt})`);
                  if (attempt < 3) await new Promise(r => setTimeout(r, 3000 * attempt));
                }
              } catch (e: any) {
                console.error(`[EcoRetry] Γ¥î Auto-continue error (attempt ${attempt}): ${e?.message}`);
                if (attempt < 3) await new Promise(r => setTimeout(r, 3000 * attempt));
              }
            }
          }
...
// ΓöÇΓöÇΓöÇ Helper: Meta API Request ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Send Text/Media Message ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Send Template Message ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Mark as Read ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Get Business Profile ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Update Business Profile ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Create Template ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Get Templates ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Delete Template ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Get Media URL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Upload Media ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Get Phone Numbers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Test Connection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Send Single Message with Retry (handles rate limits) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
async function sendWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Ecosystem errors are quality-based ΓÇö retrying immediately won't help
...
// ΓöÇΓöÇΓöÇ Insert broadcast messages into wa_messages (Live Chat) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async function insertChatMessages(
  ...
  try {
    // ΓöÇΓöÇΓöÇ Lookup template content from DB ΓöÇΓöÇΓöÇ
    let templateBody = "";
    ...
    try {
      ...
      // Template lookup failed ΓÇö fall back to basic content
    }
    ...
    if (missingPhones.length > 0) {
      ...
      const preview = (templateBody || `≡ƒôó Broadcast: ${template_name}`).substring(0, 100);
      ...
      const inserts = missingPhones.map(phone => ({
        ...
        last_message_preview: preview,  // ΓåÉ SET on initial creation, not just on update
        unread_count: 0,
      }));
    ...
    const msgInserts = sentItems.map(s => {
      ...
      const msg: Record<string, any> = {
        ...
        content: templateBody || `≡ƒôó Broadcast: ${template_name}`,
        template_name,
...
// ΓöÇΓöÇΓöÇ Backfill sweep: finds sent recipients missing wa_message entries ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Send Broadcast ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
async function sendBroadcast(
  ...
  // ΓöÇΓöÇΓöÇ SAFETY: 60s wall-clock limit ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Pre-upload media to WhatsApp ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ Pre-upload media to WhatsApp (with caching across invocations) ΓöÇΓöÇΓöÇ
          ...
          if (mediaObj && mediaObj.link && !mediaObj.id) {
            ...
            if (resolvedMediaId) {
              ...
              try {
                ...
                // Replace link with id ΓÇö WhatsApp serves the file from their CDN now
                delete mediaObj.link;
                mediaObj.id = resolvedMediaId;
              } catch (uploadErr: any) {
  ...
  // ΓöÇΓöÇΓöÇ Filter out unsubscribed customers (is_deleted = true) ΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇΓöÇ PREMIUM BROADCAST ENGINE ΓöÇΓöÇΓöÇ
  ...
  //    over 15 batches. Meta flags sudden bursts ΓÇö gradual ramp avoids it.
  //
  ...
  //    - If >10% ecosystem fail rate in rolling window ΓåÆ full pause 20s
  //    - If 8+ consecutive ecosystem fails ΓåÆ emergency pause 30s
  //
  // 3. ROLLING WINDOW: Track last 100 sends for real-time decisions.
  //
  // 4. ECOSYSTEM FAILURES ΓåÆ DEFERRED: Keep as 'pending' for next auto-continue
  //    invocation. These are quality-based, not transient ΓÇö retrying hours later
  ...
  let batchesSent = 0;
  if (slowMode) {
    console.log(`[Broadcast] ≡ƒÉó SLOW MODE active (2 msg/s) - retrying failed recipients carefully`);
  } else {
    console.log(`[Broadcast] ≡ƒÜÇ FAST MODE active (40+ msg/s) - maximum speed`);
  }
  ...
  for (let i = 0; i < filteredRecipients.length; i += concurrency) {
    // ΓöÇΓöÇΓöÇ Time check BEFORE each batch ΓöÇΓöÇΓöÇ
    const elapsed = Date.now() - functionStartTime;
    if (elapsed >= MAX_EXECUTION_MS) {
      const remaining = filteredRecipients.length - i;
      console.log(`[Broadcast] ΓÅ▒∩╕Å TIME CHECK (${(elapsed/1000).toFixed(0)}s/${(MAX_EXECUTION_MS/1000).toFixed(0)}s). ${remaining} pending. Auto-continuing...`);
      timedOut = true;
      break;
    }
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      if (res.status === "fulfilled") {
        ...
        if (isEcosystem) {
          // ΓöÇΓöÇΓöÇ ECOSYSTEM ERROR: Defer, but cap retries to prevent infinite loops ΓöÇΓöÇΓöÇ
          batchEcosystemFails++;
          ecosystemFailCount++;
    ...
    // ΓöÇΓöÇΓöÇ IMMEDIATE DB flush ΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇΓöÇ Insert sent messages into Live Chat ΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇΓöÇ Update broadcast counts ΓöÇΓöÇΓöÇ
    await supabase
      .from("wa_broadcasts")
      .update({
    ...
    // Target: 10K msgs / 10 min ΓåÆ ~17 msg/s ΓåÆ cruise at 20 msg/s
    // ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
    ...
    // ΓöÇΓöÇΓöÇ ECOSYSTEM THROTTLE ΓöÇΓöÇΓöÇ
    if (batchEcosystemFails > 0) {
      const now_ts = Date.now();

      if (windowRate > 0.10 || consecutiveEcosystemFails >= 8) {
        // CRITICAL: >10% ecosystem fail rate or 8+ consecutive ΓåÆ emergency pause (capped at 10s)
        const pauseMs = Math.min(10000, 3000 * Math.ceil(consecutiveEcosystemFails / 4));
        console.log(`[Broadcast] ≡ƒ¢æ ECOSYSTEM CRITICAL: ${(windowRate*100).toFixed(0)}% fail rate, ${consecutiveEcosystemFails} consecutive. Pausing ${pauseMs/1000}s (capped)`);
        ...
        if (windowRate > 0.15 || consecutiveEcosystemFails >= 10) {
          console.log(`[Broadcast] ≡ƒö┤ SEVERE ecosystem issues detected. Invoking auto-continue early.`);
          timedOut = true;
          break; // Exit loop to force auto-continue
        }
      } else if (windowRate > 0.05 || consecutiveEcosystemFails >= 3) {
        // MODERATE: >5% fail rate or 3+ consecutive ΓåÆ slow down significantly
        const oldC = concurrency;
        concurrency = Math.max(MIN_CONCURRENCY, Math.floor(concurrency * 0.5));
        delayMs = Math.min(2000, delayMs + 300);
        lastEcosystemPause = now_ts;
        console.log(`[Broadcast] ΓÜá∩╕Å ECOSYSTEM THROTTLE: ${batchEcosystemFails} in batch, ${(windowRate*100).toFixed(0)}% rate. c:${oldC}ΓåÆ${concurrency}, dΓåÆ${delayMs}ms`);
        await new Promise(r => setTimeout(r, 5000));
      } else {
        // LIGHT: occasional ecosystem error ΓÇö minor slowdown
        const oldC = concurrency;
        concurrency = Math.max(MIN_CONCURRENCY, concurrency - 2);
        delayMs = Math.min(1500, delayMs + 100);
        console.log(`[Broadcast] ΓÜá∩╕Å ECOSYSTEM LIGHT: ${batchEcosystemFails} in batch. c:${oldC}ΓåÆ${concurrency}, dΓåÆ${delayMs}ms`);
        await new Promise(r => setTimeout(r, 2000));
      }
    }
    // ΓöÇΓöÇΓöÇ RATE LIMIT THROTTLE ΓöÇΓöÇΓöÇ
    else if (batchRateLimited > 0) {
      const oldC = concurrency;
      concurrency = Math.max(MIN_CONCURRENCY, Math.floor(concurrency * 0.5));
      delayMs = Math.min(2000, delayMs + 500);
      console.log(`[Broadcast] Rate limited ${batchRateLimited}x, c:${oldC}ΓåÆ${concurrency}, dΓåÆ${delayMs}ms`);
      await new Promise(r => setTimeout(r, 5000));
    }
    // ΓöÇΓöÇΓöÇ WARM-UP RAMP (5ΓåÆ8ΓåÆ11ΓåÆ14ΓåÆ17ΓåÆ20 concurrency, 800ΓåÆ300ms delay) ΓöÇΓöÇΓöÇ
      ...
      if (targetC > concurrency || targetDelay < delayMs) {
        concurrency = targetC;
        delayMs = targetDelay;
        console.log(`[Broadcast] ≡ƒôê RAMP-UP ${batchesSent}/${RAMP_UP_BATCHES}: cΓåÆ${concurrency}, dΓåÆ${delayMs}ms (~${(concurrency / (delayMs/1000 + 0.3)).toFixed(0)} msg/s)`);
      }
    }
    // ΓöÇΓöÇΓöÇ RECOVERY after ecosystem pause ΓöÇΓöÇΓöÇ
    else if (Date.now() - lastEcosystemPause > 10000 && concurrency < MAX_CONCURRENCY && windowRate < 0.03) {
      // 10s since last pause + <3% fail rate ΓåÆ ramp back up aggressively
      const oldC = concurrency;
      concurrency = Math.min(MAX_CONCURRENCY, concurrency + 3);
      delayMs = Math.max(300, delayMs - 150);
      console.log(`[Broadcast] ≡ƒôê RECOVERY: c:${oldC}ΓåÆ${concurrency}, dΓåÆ${delayMs}ms (eco rate: ${(windowRate*100).toFixed(0)}%)`);
    }
  ...
  if (timedOut) {
    // ΓöÇΓöÇΓöÇ Partial completion ΓÇö outer .then() handler will auto-continue ΓöÇΓöÇΓöÇ
    ...
    // ΓöÇΓöÇΓöÇ Full completion ΓÇö backfill missing chat messages then recount ΓöÇΓöÇΓöÇ
    console.log(`[Broadcast] Γ£à COMPLETE in ${totalTime}s (${avgRate} msg/s) | sent:${sentCount} eco_defer:${ecosystemFailCount} failed:${failedCount} skipped:${skippedCount}`);
...
// ΓöÇΓöÇΓöÇ Slow Ecosystem Retry ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
async function sendEcoRetry(
  ...
  const ECO_DELAY_MS = 1500;      // 1.5s between batches ΓåÆ ~3 msg/s overall
  ...
  // ΓöÇΓöÇΓöÇ Pre-upload media (reuse cached_media_id if available) ΓöÇΓöÇΓöÇ
  let processedComponents = components;
  let resolvedMediaId = cached_media_id || null;
  ...
  for (let i = 0; i < recipients.length; i += ECO_CONCURRENCY) {
    const elapsed = Date.now() - functionStartTime;
    if (elapsed >= MAX_EXECUTION_MS) {
      console.log(`[EcoRetry] ΓÅ▒∩╕Å TIME LIMIT (${(elapsed/1000).toFixed(0)}s). ${recipients.length - i} remaining. Will auto-continue.`);
      timedOut = true;
      break;
    }
  ...
  console.log(`[EcoRetry] ${timedOut ? 'PARTIAL' : 'Γ£à COMPLETE'} in ${totalTime}s | sent:${sentCount} failed:${failedCount}`);
